...
## 9 September 2026: comparison, Topkapi and two-day itinerary

The owner requested all three proposed content additions. The four existing combo pages now compare reviewed separate-ticket and combo totals for 1–6 adults and two or three paid sights. `combo-budget.mjs` calculates with integer cents; visible, linked starting totals remain usable without JavaScript. Price snapshots are embedded as `combo-comparison-data` and checked against `offers.json` in `scripts/combo-budget.test.mjs`. When updating prices, revise the registry, tracking catalogue, all four snapshots, visible fallback prices, written examples and calculation dates together. This is not a live quote or a comparison of every seller.

`/topkapi-palace-ticket/` was materially rewritten with the official foreign visitor tariff, Harem comparison, a correctly captioned gate photo, hosted entry steps and original planning guidance. Its temporary noindex was removed and it was added to the sitemap. The two previously deferred Topkapi price/guided URLs now use the existing permanent HTML redirect mechanism to this reviewed destination. The Sultanahmet tour exclusions and deferred mappings remain in place.

`/istanbul-in-2-days/` is a new indexable, self-canonical English guide. It splits the three Old City sights across two days, includes rest and transfer allowances, distinguishes cruise piers and incorporates Tuesday/Friday restrictions. The route estimates are editorial, not supplier schedules or an account of an on-site test. Both guides are linked from the homepage, guide directory, planner and existing related guides. New guide cards have authored categories respected by the directory filter.

IWC's rendered Included tabs now explicitly confirm that the standalone Topkapi audio ticket and both Old City combos exclude the Harem. Earlier “confirm inclusion” language was replaced at the decision points, including localized combo pages. The official venue ticket includes different palace sections and stays denominated in TL; no currency conversion or identical-inclusions claim is made. New IWC Topkapi and standalone cruise offers preserve the owner-supplied referral and have separate consent-gated Analytics events. The unused historical GetYourGuide Topkapi registry records are retained; validator warnings for those two records are expected.

Sources reviewed: National Palaces Topkapi location page; DEM public visitor FAQ (Friday restrictions, not agency purchase terms); Blue Mosque visitor guide; Metro Istanbul T1 route; rendered IWC Topkapi, Old City Combo, Saver and standalone Bosphorus cruise product tabs. All substantive changes use 9 September dates in the sitemap and Article metadata. Existing scripts verify links, attribution, structured data and consolidation consistency. New budgeting tests cover arithmetic, different sight coverage, group totals, reversals and language snapshot drift.

## Reviewed multilingual pages (9 September 2026)

The DE, FR and ES homepages use the English homepage structure. The visitor planner, photo map, printed route and downloadable pocket card are localized too. The new Topkapi, two-day Istanbul, Basilica, Friday and Galataport guides have dedicated language equivalents. Existing consolidated URLs stay redirected; the localized guide indexes are now complete and indexable.

- `scripts/locales/source-strings.json` and `ui.tsv` contain reviewed homepage/planner translations. IDs are stable; append new IDs rather than renumbering. The builder fails on unrecognized template copy.
- `scripts/locales/articles.tsv` contains localized article copy. `pages.json` maps new equivalent routes. Do not map an unrelated article merely to populate a language selector.
- Rebuild static HTML and `visit-i18n.mjs` with `python3 scripts/build-localized-site.py` (requires `beautifulsoup4`). Run from a clean worktree and review the diff. The review date is intentionally fixed; advance it only after a substantive source/content review.
- `scripts/locales/itinerary-snapshot.json` is the full-day, three-sight editorial route from `itinerary({sights:'three', style:'self', duration:'8'})`. Refresh it when route logic changes and review the translated output.
- Generate localized pocket cards with `python3 scripts/build-localized-visit-cards.py` (requires reportlab). Set `HAGIA_CARD_FONT_DIR` to a folder containing the DejaVu Sans regular and bold TTF files if needed. Render and inspect all three PDFs after changes.
- `node --test scripts/*.test.mjs` checks all planner choices in every language, affiliate behavior and equivalent-page navigation. `node scripts/validate-site.mjs` also checks local assets, links, canonical/hreflang targets, sitemap inclusion and matching modification dates.
- Affiliate destinations retain `ref=iti5`; referral mechanics must never appear in visitor-facing text. Events retain the same product IDs across languages and record the page language only after analytics consent.

Submit `https://hagiasophiaticket.com/sitemap.xml` in Search Console's Sitemaps report if it is not already listed. This one root sitemap covers all four languages; robots.txt already points to it. Only indexable canonical HTML belongs in the sitemap. Submission does not guarantee indexing.
